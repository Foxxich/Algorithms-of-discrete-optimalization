...
int createPointsFileResult(FILE *file, Graph *g, Points *p, const char* graphFile, const char* pointsFile, double time)
{
...
int optionSourcesList(Graph *g, void (*algorithm)(Graph*,int,int,char), const char *inputFile, const char *resultFile, const char *graphFile)
{
    FILE *file;
    Sources s;
    int i;
...
int optionPointsList(Graph *g, void (*algorithm)(Graph*,int,int,char), const char *inputFile, const char *resultFile, const char *graphFile)
{
    Points p;
    int i;
    FILE *file;
    ...
    for(i=0; i<p.size; i++)
    {
        //printf("%d %d \n", p.points[i].source, p.points[i].destination);
        clock_t t = clock();
        algorithm(g, p.points[i].source, p.points[i].destination, 'p');
        p.points[i].distance = g->nodeList[p.points[i].destination - 1].value;
        sum += clock() - t;
        //printf("%d\n", p.points[i].distance );
        resetGraph(g);
    }
    double avgTime = ((double)sum)/(CLOCKS_PER_SEC*p.size);
    if((file = fopen(resultFile, "w")) == NULL)
    {
        printf("Error occured while creating result file: %s\n", resultFile);
        return 1;
    }
    createPointsFileResult(file, g, &p, graphFile, inputFile, avgTime);
    fclose(file);
    return 0;
}
...
int generateResult(int argc, char **argv, void (*algorithm)(Graph*,int,int,char))
{
    int i;
